...
namespace GMS.Account.BLL
{
    public class AccountService : IAccountService
    {
        ...
        public void ModifyPwd(int userID, String oldPassword, String newPassword)
        {
            using (var dbContext = new AccountDbContext())
            {
                var user = dbContext.Users.SingleOrDefault(x => x.ID == userID);
                if (user != null && user.Password == Encrypt.MD5(oldPassword))
                {
                ...
                {
                    throw new BusinessException("密码不一致，请重新确认密码！");
                }
            }
        }
